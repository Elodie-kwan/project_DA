...
At the present time the variable that seems to be necessary for classification is **Checking account**. The fact that this variable predominates over the others may serve to simplify the determination model for Good or Bad credit risk. However, it should be kept in mind that the database is unbalanced in favour of bad credits, which makes it difficult to know the real cause of their default. In order to be able to apply models that know the cause and not the correlation between risky credits and client profiles, we recommend to collect more data on them. 
