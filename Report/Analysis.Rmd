...
```{r}
German_credit.num.response <- German_credit %>% 
  select('RESPONSE','DURATION', 'AMOUNT', 'INSTALL_RATE', 'AGE', 'NUM_CREDITS','NUM_DEPENDENTS')
```

```{r}
German_credit.knn <- knn3(data=German_credit.num.response, RESPONSE~., k=2) ## build the K-NN model

German_credit.pred <- predict(German_credit.knn, newdata = German_credit.num.response, type="class")
...
```{r}
German_credit.knn <- knn3(data=German_credit.tr, RESPONSE~., k=2)  # k = 2

German_credit.te.pred <- predict(German_credit.knn, newdata = German_credit.te, type="class")
```

To compare the predictions above and the true species (the one in the test set), we can build a table. It is called a **confusion matrix** (again, this will be detail later on).

```{r}
table(Pred=German_credit.te.pred, Observed=German_credit.te[,25])
```
The table is read as follow : 
+ We predicted 13 Bad credits and there were indeed 13 observed Bad credits. But the prediction misjudge 45 good credits by predicting bad credits.
...
```{r}
German_credit.knn <- knn3(data=German_credit.tr, RESPONSE~., k=3)  # k = 3
German_credit.te.pred <- predict(German_credit.knn, newdata = German_credit.te, type="class")
...
```{r}
library(e1071)
German_credit.svm <- svm(RESPONSE ~ ., data=German_credit.tr, kernel="linear")
German_credit.svm
...
```{r}
German_credit.rb <- svm(RESPONSE ~ ., data=German_credit.tr, kernel="radial")
German_credit.rb
...
```
The accuracy is lower, 71.2%, compared to 74% with the linear method.
...
```{r}
set.seed(143)
svm_Linear <- train(RESPONSE ~., data = German_credit.tr, method = "svmLinear",
...
set.seed(143)
svm_Linear_Grid <- train(RESPONSE ~., data = German_credit.tr, method = "svmLinear",
                           trControl=German_Credit.trctrl,
                           tuneGrid = grid)
svm_Linear_Grid
...
German_credit.boot <- train(RESPONSE ~., data = German_credit.tr, method = "glmStepAIC", family="binomial",
